  ...
  return (
    <AdminLayout>
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {unassignTarget && (
          <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => !unassign.isPending && setUnassignTarget(null)}>
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-slate-900">Unassign instructor?</h3>
                  <p className="text-sm text-slate-500 mt-1">
                    Remove <span className="font-medium text-slate-700">{unassignTarget.instructorName}</span> from{" "}
                    <span className="font-medium text-slate-700">{unassignTarget.courseTitle}</span>?
                  </p>
                </div>
              </div>

              <label className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200 cursor-pointer mb-5">
                <input
                  type="checkbox"
                  checked={unassignNotify}
                  onChange={(e) => setUnassignNotify(e.target.checked)}
                  className="mt-0.5 accent-amber-600"
                />
                <div>
                  <div className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                    <Bell className="w-3.5 h-3.5 text-amber-600" /> Notify enrolled students
                  </div>
                  <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                    <Mail className="w-3 h-3" /> Sends an in-app notification + email so students know a new instructor is coming.
                  </div>
                </div>
              </label>
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {unassign.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                  {unassignNotify ? "Unassign & Notify" : "Unassign"}
                </button>
              </div>
            </div>
          </div>
