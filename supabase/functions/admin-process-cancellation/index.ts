...
// On approve: cancels enrollment + (optionally) creates Paddle refund adjustment.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (issueRefund && enrollment?.stripe_payment_intent) {
      try {
        const txnId = enrollment.stripe_payment_intent;
        // Fetch transaction to get the item id required by /adjustments
        const txnRes = await gatewayFetch(environment, `/transactions/${encodeURIComponent(txnId)}`);
        const txnJson = await txnRes.json();
        const items = txnJson?.data?.details?.line_items
          ?? txnJson?.data?.items
          ?? [];
    ...
    await admin.from('ssra_cancellation_requests').update({
      status: refundError ? 'approved' : (paddleAdjustmentId ? 'refunded' : 'approved'),
    ...
    await admin.from('ssra_notifications').insert({
      ...
      body: paddleAdjustmentId
        ? `Your enrollment was cancelled and a refund has been issued via Paddle. It may take 5–10 business days to appear on your statement.`
    ...
    return new Response(JSON.stringify({
      ok: true,
      status: paddleAdjustmentId ? 'refunded' : 'approved',
      paddleAdjustmentId,
      refundError,
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
